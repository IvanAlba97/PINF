...
void strcatSol (Tcomsol* sol) {
	...
	if (sol->bpm) {
	  ...
	  strcpy(cad,"");
          ...
          sprintf (cad,"<TR><TD><b>Altura verdadera:</b></td><td>%2iº %4.1lf\'</td></tr>", sol->gra, sol->mia);
	  printf("%s",cad);
	...
	if (sol->hss[0] == 99) {
	  ...
	  printf("</TABLE>");
		...
		printf("</TABLE>");
      		...
      		printf("<TABLE><TR><TD><B>No hay fen&oacute;menos.</B></TD></TR>");
		printf("<TR><TD><B>Sol siempre con distancia cenital z > 102º.</B></TD></TR>");
		printf("</TABLE>");
	}
...
/*	  if (!sol->bpm){
	  ...
	  if (sol->hss[0] != -1 || sol->hps[0] != -1){
	    ...
	    // Acimutes de salida 1er fenomeno
            ...
            if (sol->hss[0] != -1){
              printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD>",sol->ssg[0], sol->ssm[0], sol->hss[0], sol->mss[0]);
	    }
	    else{
              printf("<TD> --------- </TD>");
	    }
	    if (sol->hsi[0] != -1){
              printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD></TR>", sol->sig[0], sol->sim[0], sol->hsi[0], sol->msi[0]);
	    }
	    else{
              printf("<TD> --------- </TD></TR>");
	    }
	    ...
	    if ((sol->hss[0] != -1 && sol->hss[1] != -1) || (sol->hsi[0] != -1 && sol->hsi[1] != -1)) {
	      ...
	      if (sol->hss[1] != -1){
                printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD>", sol->ssg[1], sol->ssm[1], sol->hss[1], sol->mss[1]);
	      }
	      else{
                printf("<TD> --------- </TD>");
	      }
	      if (sol->hsi[1] != -1){
                printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD></TR>", sol->sig[1], sol->sim[1], sol->hsi[1], sol->msi[1]);
	      }
	      else{
                printf("<TD> --------- </TD></TR>");
	      }
	    ...
	    if (sol->hps[0] != -1){
              printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD>", sol->psg[0], sol->psm[0], sol->hps[0], sol->mps[0]);
	    }
	    else{
              printf("<TD> --------- </TD>");
	    }
	    if (sol->hpi[0] != -1){
              printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD></TR>", sol->pig[0], sol->pim[0], sol->hpi[0], sol->mpi[0]);
	    }
	    else{
              printf("<TD> -------- </TD></TR>");
	    }
	    ...
	    if ((sol->hps[0] != -1 && sol->hps[1] != -1) || (sol->hpi[0] != -1 && sol->hpi[1] != -1)) {
              printf("<TR><TD><B>y</B></TD>");
	      if (sol->hps[1] != -1){
                printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD>",sol->psg[1], sol->psm[1], sol->hps[1], sol->mps[1]);
	      }
	      else{
                printf("<TD> --------- </TD>");
	      }
	      if(sol->hpi[1] != -1){
                printf("<TD>%3iº%4.1f' a %2ih %2.0fm</TD></TR>", sol->pig[1], sol->pim[1], sol->hpi[1], sol->mpi[1]);
	      }
	      else{
                printf("<TD> --------- </TD></TR>");
	      }
