...
double angpol ( double fi, double de, double dz );
 void hodobfen ( double ut[2], double dj, double huso, int *h, double *m, double err);
...
int main(void)
{
   ...
   else if(sscanf(data,"latgra=%i&latmin=%i&latseg=%i&latsig=%c&longra=%i&lonmin=%i&lonseg=%i&lonsig=%c&elev=%lf&horut=%i", \
&latgra, &latmin, &latseg, &latsig, &longra, &lonmin, &lonseg, &lonsig, &elev, &horut) != 10)
   	printf("<P>Error al pasar los datos al programa.");
   else
